...
import { lineItemTotal, listLineItemsForSubscription } from '../lib/lineItemQueries.js';
...
function rowToSubscription(r: typeof subscription.$inferSelect): Subscription {
  return {
    id: r.id,
...
export function getSubscription(db: Db, subscriptionId: string): Subscription | null {
  const row = db.select().from(subscription).where(eq(subscription.id, subscriptionId)).get();
  return row ? rowToSubscription(row) : null;
}

export function listActiveSubscriptionsForCustomer(db: Db, customerId: string): Subscription[] {
  const rows = db
    .select()
    .from(subscription)
